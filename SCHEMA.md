...
Generated by `scripts/build-geo.py`, stdlib only and re-runnable, from three inputs: the Statistics Canada 2021 Census **cartographic** boundary files for forward sortation areas (`lfsa000b21a_e`) and census divisions (`lcd_000b21a_e`), both downloaded 2026-09-10, and `data/ns-community-points.json`. The source zips (162 MB and 140 MB) live in `data/geo-src/` and are gitignored; only this output, about 160 KB, ships.

**Why the cartographic files, and what was wrong before.** The first map (2026-09-04) used the *digital* boundary file `lfsa000a21a_e`, whose coastal boundaries run out into the water instead of following the shoreline. Its Nova Scotia shapes covered 76,209 km² against 52,825 km² of land, 44% too much, so the Minas Basin, the Bras d'Or Lakes and the bays were filled in. Feedback on 2026-09-10 said the borders did not match the real map. The cartographic files follow the shoreline. The remaining excess, 8% over land, sits in lake-dense Halifax postal areas, where lakes legitimately stay inside the boundary.

Nova Scotia only, per Irene's scope decision 2026-09-04: 77 postal areas, 14 of them in the rural study frame. The projection is handled as before: StatCan Lambert is inverted to latitude and longitude in closed form, then re-projected with a local Mercator so the province sits in its familiar orientation.

| Field | Notes |
|---|---|
| `viewBox` | SVG viewBox for the whole province, width normalised to 1000 |
| `features[].fsa` | Postal area code, joins to `regions.json` on `fsa` |
| `features[].in_frame` | `true` for codes whose second character is `0`, the rural frame named in the workbook methodology |
| `features[].counties` | County make-up of the area by land share, measured by sampling about 600 grid points inside it against the county boundaries. Shares under 5% are dropped as boundary noise |
| `features[].d` | SVG path data, islands included. Pieces under 1 km² are dropped, except where that would remove a whole area: B3J, downtown Halifax, keeps its largest piece |
| `county_lines` | Internal county borders only: edges two county polygons share exactly, 4,202 of them chained into 39 lines, so the coastline is drawn once rather than twice slightly out of register |
| `county_labels` | County name and position, moved clear of community dots wherever they would overlap |
| `points` | Each dashboard community at its official location, with its county and the postal area that contains it. Pictou and New Glasgow have official points on the waterfront just past the simplified shoreline, and snap to their area within 3 km |

**Region names on the dashboard now come from this file, not from the workbook.** The DER sheet's Region / County column labels each postal area by hand, and measured against the county boundaries several of those labels name the wrong counties:

| Postal area | Workbook label | County make-up by land area |
|---|---|---|
| B0W | Shelburne / Yarmouth | Digby 47%, Yarmouth 37%, Shelburne 16% |
| B0J | Lunenburg / Guysborough | Halifax 35%, Lunenburg 29%, Guysborough 29%, Queens 6% |
| B0N | Colchester / Kings / Hants | Hants 52%, Halifax 29%, Colchester 19% |
| B0C | Cape Breton (rural north) | Victoria 100% |
| B0K | Pictou | Pictou 73%, Colchester 16%, Cumberland 12% |
| B0R | Lunenburg | Lunenburg 65%, Annapolis 35% |
| B0V | Digby / Annapolis | Digby 99% |
| B0E | Richmond / Inverness / Cape Breton (south) | Inverness 64%, Richmond 17%, Victoria 12%, Guysborough 6% |
| B0P | Kings / Annapolis | Kings 87%, Hants 9% |

B0H, B0L, B0M, B0S and B0T match. The map panel and the detail panel use the measured make-up; `region_county` in `regions.json` still carries the workbook label, unchanged, because the workbook is the system of record.

## data/ns-community-points.json

Input, committed. The official point for each of the 19 Nova Scotia communities, from Natural Resources Canada's Canadian Geographical Names Database (geogratis geonames API), retrieved 2026-09-10. Town records where one exists. **Richmond** is the Municipality of the County of Richmond, not the hamlets of that name in Yarmouth or Cumberland, which matches its 3,960 households in the Need Index. **West Hants** is the West Hants Regional Municipality.

Used to draw each community where it really is, and to check it sits inside the postal area the dashboard assigns it. **All 18 with a postal code do.** Richmond has no postal code in the workbook; its official point falls inside B0E, and it is placed there on the map only. No retrofit context is borrowed for it: its `fsa` stays null in `regions.json`.
